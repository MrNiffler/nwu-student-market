  ...
  try {
    ...
    if (!category_id && !type && !description && !price && !status) {
      return res.status(400).json({
        success: false,
        message:
          "Please provide at least one field to update (category_id, type, description, price, or status).",
      });
    }
...
router.post("/:id/images", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (ownershipCheck.rows.length === 0) {
      ...
      return res.status(404).json({
        success: false,
        message: "Listing not found or you do not have permission to add images.",
      });
    }
    ...
    const result = await pool.query(query, [id, imageUrl, altText]);
...
router.delete("/:id/images/:imageId", async (req, res) => {
  ...
  try {
    ...
    const imageResult = await pool.query(imageQuery, [imageId, id]);
    ...
    const deleteResult = await pool.query(deleteQuery, [imageId, id]);
